...
mono --debug ../brace/target5/BEX_E_mcs.exe ../brace/source/base/Uses.be --buildFile build/shared.txt --deployPath deployKh --buildPath targetKh --emitLang cs --outputPlatform linux -mainClass=Konnectii:Host ../brace/source/extended/Log.be source/IUKH.be

lae=$?;if [[ $lae -ne 0 ]]; then exit $lae; fi

mcs -debug:pdbonly -warn:0 -out:targetKh/BEX_E_mcs.exe ../brace/system/cs/be/*.cs targetKh/Base/target/cs/be/*.cs
